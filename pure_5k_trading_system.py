#!/usr/bin/env python3
"""
🚀 PURE $5K ULTRA-AGGRESSIVE TRADING SYSTEM - NO DAILY ADDITIONS
===============================================================
...
class Pure5KTradingSystem:
    ...
    def cache_historical_data(self, days: int = 90) -> None:
        """Download and cache historical data with proper timezone handling"""
        print(f"\n📥 Caching {days} days of historical data for offline use...")
        
        cache_file = f"pure_5k_cache_{days}days.pkl"
    ...
    def get_price_from_cache(self, symbol: str, date: str = None) -> float:
        """Get price from cached historical data with proper timezone handling"""
        ...
        try:
            ...
            if date:
                # Standardize target date to UTC
                target_date = self.standardize_datetime(date)
                
                # Find data within reasonable time window
                time_diff = abs(hist_data.index - target_date)
                closest_idx = time_diff.argmin()
                
                # Only use if within 2 days
                if time_diff.iloc[closest_idx] <= pd.Timedelta(days=2):
    ...
    def get_current_price_online(self, symbol: str, date: str = None) -> float:
        ...
        if symbol in self.historical_data_cache:
            try:
                return float(self.historical_data_cache[symbol]['data']['Close'].iloc[-1])
            except:
                pass
...
def main():
    ...
    try:
        ...
        results_file = f"pure_5k_results_{timestamp}.json"
