...
class AITradingPredictor:
    ...
    def calculate_technical_features(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            # Base features that will always be available
            base_features = []
            ...
            for period in self.lookback_periods:
                if len(data) > period:
                    ...
                    base_features.append(f'return_{period}d')
                    
                    # Moving averages
                    features[f'sma_{period}'] = data['Close'].rolling(period).mean()
                    features[f'ema_{period}'] = data['Close'].ewm(span=period).mean()
                    base_features.extend([f'sma_{period}', f'ema_{period}'])
                    
                    # Price position relative to MA
                    features[f'price_vs_sma_{period}'] = data['Close'] / features[f'sma_{period}'] - 1
                    base_features.append(f'price_vs_sma_{period}')
                    
                    # Volatility
                    features[f'volatility_{period}d'] = data['Close'].pct_change().rolling(period).std()
                    base_features.append(f'volatility_{period}d')
            ...
            base_features.extend(['macd', 'macd_signal', 'macd_histogram'])
            ...
            if len(data) > bb_period:
                ...
                base_features.extend(['bb_upper', 'bb_lower', 'bb_position'])
            ...
            base_features.extend(['price_momentum_5', 'price_momentum_10'])
            ...
            base_features.extend(['higher_highs', 'lower_lows'])
            
            # Volume features (only if available)
            if 'Volume' in data.columns and data['Volume'].notna().any():
                for period in self.lookback_periods:
                    ...
                    base_features.extend([f'volume_sma_{period}', f'volume_ratio_{period}'])
            else:
                # Add dummy volume features to maintain consistent feature count
                for period in self.lookback_periods:
                    features[f'volume_sma_{period}'] = 0
                    features[f'volume_ratio_{period}'] = 1
                    base_features.extend([f'volume_sma_{period}', f'volume_ratio_{period}'])
            
            # Store base feature list for consistent feature selection
            self.base_features = base_features
            
            # Return only the base features in a consistent order
    ...
    def predict_signal(self, symbol: str, data: pd.DataFrame) -> Dict:
        ...
        try:
            ...
            # Calculate features using the same base feature set
            features_df = self.calculate_technical_features(data)
            if len(features_df.columns) == 0:
                return {'signal': 'NEUTRAL', 'confidence': 0.0, 'predicted_return': 0.0}
            ...
            result = {
                ...
                'features_used': len(self.base_features)
            }
...
class Pure5KV4AITradingSystem:
    ...
    def execute_ai_initial_allocation(self, date: str):
        ...
        if self.ai_enabled:
            for symbol in self.all_symbols:
                if symbol in self.historical_data_cache:
                    ...
                    if prediction.get('confidence', 0) > 0.6:  # At least one strong signal
                        ai_working = True
        
        # Fallback: If AI isn't working or all signals are weak
        if not ai_working or total_confidence / len(self.all_symbols) < 0.4:  # Average confidence < 40%
            print("⚠️ AI signals weak, using fallback allocation strategy")
            ai_signals = self.generate_fallback_signals(date)
            ai_working = True  # Force allocation
        
        # Allocate based on AI confidence and sector allocation
        total_invested = 0.0
        min_allocation = 0.05  # Minimum 5% allocation per asset
        
        # First pass: Calculate desired allocations
        desired_allocations = {}
        valid_symbols = []
        ...
        for symbol in self.all_symbols:
            try:
                ...
                if price <= 0:
                    self.logger.warning(f"No valid price for {symbol} on {date}")
                    continue
                ...
                sector_budget = self.initial_balance * base_allocation
                position_value = sector_budget * ai_multiplier * ai_signal.get('suggested_position_size', 0.15)
                position_value = max(min(position_value, self.max_position_value), 
                                   self.initial_balance * min_allocation)  # At least 5% allocation
                ...
                self.logger.error(f"Failed to calculate allocation for {symbol}: {e}")
                continue
        
        if not valid_symbols:
            print("❌ No valid symbols found for allocation")
            return
        
        # Second pass: Normalize allocations to total 100%
        total_desired = sum(alloc['value'] for alloc in desired_allocations.values())
        scale_factor = self.initial_balance / total_desired if total_desired > 0 else 0
        
        # Execute trades
        for symbol in valid_symbols:
            try:
                alloc = desired_allocations[symbol]
                price = alloc['price']
                
                # Scale the position size
                position_value = alloc['value'] * scale_factor
    ...
    def generate_fallback_signals(self, date: str) -> Dict:
        ...
        # First pass: Calculate momentum and technical scores
        scores = {}
        for symbol in self.all_symbols:
            if symbol in self.historical_data_cache:
                try:
                    ...
                    # RSI calculation for oversold/overbought
                    delta = data['Close'].diff()
                    gain = (delta.where(delta > 0, 0)).rolling(window=14).mean()
                    loss = (-delta.where(delta < 0, 0)).rolling(window=14).mean()
                    rs = gain / loss
                    ...
                    momentum_score = min(max(recent_return / 10, -1), 1)  # Scale -10% to +10% to -1 to 1
                    volume_score = min(volume_ratio - 1, 1)  # Volume above average
                    rsi_score = abs(50 - current_rsi) / 50  # Distance from neutral RSI
                    
                    # Combined score
                    total_score = (momentum_score + volume_score + rsi_score) / 3
        ...
        # Second pass: Normalize scores and generate signals
        if scores:
            max_score = max(s['total'] for s in scores.values())
            min_score = min(s['total'] for s in scores.values())
            score_range = max_score - min_score if max_score != min_score else 1
            
            for symbol, score in scores.items():
                # Normalize score to 0-1 range
                normalized_score = (score['total'] - min_score) / score_range
                
                # Ensure we always have some allocation
                confidence = 0.3 + (normalized_score * 0.5)  # Range: 0.3 to 0.8
                
                # Position size based on relative strength
                position_size = 0.05 + (normalized_score * 0.15)  # Range: 5% to 20%
                
                # Signal type based on momentum
                if score['recent_return'] > 3 or normalized_score > 0.8:
                    signal = 'STRONG_BUY'
                elif score['recent_return'] > 0 or normalized_score > 0.5:
                    signal = 'BUY'
                else:
                    signal = 'BUY'  # Force buy for initial allocation
                
                signals[symbol] = {
                    'signal': signal,
                    'confidence': float(confidence),
                    'suggested_position_size': float(position_size),
                    'predicted_return': float(score['recent_return'] / 100)
                }
        
        # Ensure minimum allocations
        if not signals:
            # Equal weight fallback if all else fails
            weight = 1.0 / len(self.all_symbols)
            for symbol in self.all_symbols:
                signals[symbol] = {
                    'signal': 'BUY',
                    'confidence': 0.5,
                    'suggested_position_size': float(weight),
                    'predicted_return': 0.01  # Small positive expectation
                }
    ...
    def execute_ai_trading_logic(self, date: str):
        ...
        # Check if AI predictions are too weak
        if total_confidence / len(self.all_symbols) < 0.4:  # Average confidence < 40%
            print("⚠️ AI signals weak, using fallback trading strategy")
            ai_signals = self.generate_fallback_signals(date)
        ...
        # Force initial allocation if we have too much cash
        if self.cash > self.initial_balance * 0.8:  # More than 80% cash
            print("💰 High cash balance detected, forcing initial allocation...")
            self.execute_ai_initial_allocation(date)
        ...
        try:
            ...
            if date:
                ...
                if 0 <= closest_idx < len(data):
                    price = float(data.iloc[closest_idx]['Close'])
                    if pd.isna(price) or price <= 0:
                        return float(data['Close'].iloc[-1])  # Fallback to latest price
                    return price
            
            # If no date specified or date lookup failed, return latest price
            latest_price = float(data['Close'].iloc[-1])
            return latest_price if not pd.isna(latest_price) and latest_price > 0 else 0.0
        except Exception as e:
            self.logger.error(f"Price lookup failed for {symbol}: {e}")
            return 0.0
